...
const d = useRoute()
console.log('login route: ', d.meta)
...
const handlerLogin = async () => {
  console.log('login   ====>  hash: ', auth.value.hash, 'password:', password.value)
  const {} = await $fetch('/net_cut/login', {
    method: 'post',
    body: {hash: auth.value.hash, password: password.value}
  })
  auth.value.isAuthenticated = true
  navigateTo(auth.value.redirect || '/')
}
...
<template>
  <div
    ...
    <div
        class="w-[25rem] h-[25rem] flex flex-col items-center justify-center bg-gray-200 rounded-full drop-shadow ring-offset-2  ring-2">
      <label class=" input input-accent flex items-center gap-2">
        <input
            v-model="password"
        ...
        />
        <kbd @click.stop="handlerLogin" class="kbd kbd-sm">Enter</kbd>
      </label>

      <div class="label w-14 h-14 overflow-hidden hover:cursor-pointer" @click.stop="handlerLogin">
        <svg t="1715413331176" class="icon icon-fill" viewBox="0 0 1024 1024" version="1.1"
...
.icon-fill {
  --tw-border-opacity: 1;
  fill: var(--fallback-a, oklch(var(--a)/var(--tw-border-opacity)));
}
